(function( $ ){
	...
	$.fn.multiple_emails = function() {
		...
		return this.each(function() {
			...
			if ($(this).val() != '') {
				$.each(jQuery.parseJSON($(this).val()), function( index, val ) {
					$list.append($('<li class="multiple_emails-email"><span class="email_name">' + val + '</span></li>')
					  .prepend($('<a href="#" class="multiple_emails-close" title="Remover"><span class="glyphicon glyphicon-remove" style="color: red;margin-right: 5px;top: 2px;"></span></a>')
			...
			var $input = $('<input type="text" class="multiple_emails-input text-center" />').keyup(function(event) { // input
				...
				if( event.which == 32 || event.which == 188 || event.which == 13) { //space or comma or enter		 
					var val;
					if( event.which == 32 || event.which == 188)
						$(this).val().slice(0, -1); // remove space/comma from value
					else
						val = $(this).val();
